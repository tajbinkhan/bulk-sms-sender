...
hourEntry = Entry(
	root,
	width=3,
	font=(font_style, 18, "normal"),
	borderwidth=0,
	highlightbackground=highlight_background_color,
...
# ------- Country Code -------#
Label(root, text="Country Code or Number", font=(font_style, 11, "normal")).grid(
	row=0, column=0, pady=(20, 0), padx=(40, 0)
)
country_code = Entry(
	root,
	width=20,
	font=(font_style, 11, "normal"),
	borderwidth=0,
	highlightbackground=highlight_background_color,
	highlightcolor=highlight_color,
	highlightthickness=1,
...
def config():
	...
	top.geometry(
		f"{config_app_width}x{config_app_height}+{int(config_x)}+{int(config_y)}"
	)
	...
	def loading_configuration():
		...
		global alpha_config
		global checkbox
		Label(top, text="Account SID", font=(font_style, 11, "normal")).grid(
			row=0, column=1, pady=(20, 0), padx=(40, 0)
		)
		v1 = StringVar(top, value=sid_data)
		account_sid_config = Entry(
			top,
			textvariable=v1,
			width=35,
			font=(font_style, 11, "normal"),
		...
		Label(top, text="Auth Token", font=(font_style, 11, "normal")).grid(
			row=2, column=1, pady=(20, 0), padx=(40, 0)
		)
		v2 = StringVar(top, value=token_data)
		auth_token_config = Entry(
			top,
			textvariable=v2,
			width=35,
			font=(font_style, 11, "normal"),
		...
		alpha_label = Label(
			top, text="Alphanumeric Code", font=(font_style, 11, "normal")
		)
	...
	def saving_configuration():
		if (
			len(account_sid_config.get()) == 0
			or len(auth_token_config.get()) == 0
			or len(sender_config.get()) == 0
		):
	...
	if os.path.exists(twilio_database):
		try:
			...
			confirmation = info_msg(
				"Reset Database",
				"Somehow database has messed up. The application has reset the database. Please fill up with credentials. Thank you!!",
			)
...
def openDialogBox():
	...
	root.filename = filedialog.askopenfilename(
		title="Choose CSV File", filetypes=(("CSV file", "*.csv"), ("All files", "*.*"))
	)
	...
	try:
		...
		def clicked_menu():
			...
			if error:
				...
				try:
					...
					get_country_code = country_code.get()
					if (
						len(get_country_code) > 3
						or str(get_country_code).startswith("+") == False
					):
						info_msg("Warning", "Invalid Country Code")
					elif len(get_country_code) == 0 or len(get_the_message) == 1:
						info_msg("Warning", "Empty number or text message!")
						...
						for i in int_numbers:
							if running:
								try:
									...
									success_status_log(
										f"{full_phone_number} - Successfully Send {send_success} contact(s)\n"
									)
									...
									if "20003" in str(e):
										...
										info_msg(
											"Error",
											"May be your Twilio Account is currently suspended due to a lack of funds!",
										)
										running = False
										...
										failed_status_log(
											f"{full_phone_number} - Failed to send {send_failed} contact(s)\n"
										)
									...
									info_msg(
										"Error",
										"Application is facing some issues. Contact with the developer!",
									)
									running = False
						...
						if total_send_success > 0 or total_send_failed > 0:
							failed_status_log("====================================\n")
							failed_status_log(
								f"Successfully send {total_send_success} contact(s). Failed to send {total_send_failed} contact(s)"
							)
							info_msg(
								"Sent Message Status",
								f"Successfully send {total_send_success} contact(s). Failed to send {total_send_failed} contact(s)",
						...
						if os.path.exists(f"{path}\\failed {file_name}.csv"):
							try:
								failed_file.close()
								if (
									os.stat(f"{path}\\failed {file_name}.csv").st_size
									<= 9
								):
									os.remove(f"{path}\\failed {file_name}.csv")
							except UnboundLocalError:
		...
		def show_contact_list():
			...
			csv_list.geometry(
				f"{csv_app_width}x{csv_app_height}+{int(csv_x)}+{int(csv_y)}"
			)
	...
	if error:
		...
		try:
			...
			if len(get_number) == 14 or len(get_number) == 12:
				if checkvar.get() == 1:
					client.messages.create(
						body=get_the_message, from_=alpha, to=get_number
					)
				else:
					client.messages.create(
						body=get_the_message, from_=sender, to=get_number
					)
...
single_sms_send = Button(
	root,
	text="Send Single Message",
	bg=button_bg_color,
	fg=button_fg_color,
	activebackground=button_bg_color,
	activeforeground=button_fg_color,
...
progress_stop_btn = Button(
	root,
	text="Stop Sending Message",
	bg=button_bg_color,
	fg=button_fg_color,
	activebackground=button_bg_color,
	activeforeground=button_fg_color,
...
output_btn = Button(
	...
	pady=5,
	padx=5,
	command=output_Folder_location,
)

# ------- Set Timer Button -------#
set_timer_btn = ttk.Checkbutton(
	frame1, text="Set Timer", takefocus=False, variable=setvar, command=set_timer
)
